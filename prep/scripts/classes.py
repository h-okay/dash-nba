...
class winProbability:
    def __init__(self, team):
        self.team = team
        with open("prep/models/winprobability/winprobamodel.pkl",
                  "rb") as file:
            self.model = pkl.load(file)

    def prep(self):
        schedule = pd.read_csv("prep/data/schedule.csv").drop("Time / TV",
                                                              axis=1)
        self.next_match = (
            schedule[
...
class MVPForecast:
    ...
    def prep_and_predict(self):
        self.data_2022 = self.cands_2022.merge(
            ...
            left_on=["Player", "Team"],
            right_on=["Player", "Tm"],
        ).drop(["W", "L", "Rk_x", "Rk_y", "Tm", "Unnamed: 19", "Unnamed: 24"],
               axis=1)
        ...
        self.data_2022.to_csv(f"prep/estimations/mvps/2022_mvp.csv",
                              index=False)
