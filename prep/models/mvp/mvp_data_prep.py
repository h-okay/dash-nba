...
for i in tqdm(range(1980, year)):
    temp1 = pd.read_html(
        f"https://www.basketball-reference.com/awards/awards_{i}.html",
        header=1,
        match="Most Valuable Player",
    )[0]
    temp1["Year"] = str(i)
    mvp_cand = mvp_cand.append(temp1, ignore_index=True)

    temp2 = pd.read_html(
        f"https://www.basketball-reference.com/leagues/NBA_{i}_advanced.html",
        header=0,
        match="Advanced",
    )[0]
...
team_standings = pd.merge(
    team_standings, teams, how="left", left_on="Team", right_on="full_name"
)
team_standings.tail(20)
