...
east_conf = ['Miami Heat', 'Chicago Bulls', 'Philadelphia 76ers', 'Cleveland Cavaliers', 'Milwaukee Bucks',
             ...
             'Washington Wizards', 'New York Knicks', 'Indiana Pacers', 'Detroit Pistons', 'Orlando Magic']
...
def champ_data_prepare(data, stage, drop_list = None):
    ...
    y_data = data[stage]
    X_data = data.drop(['TEAM', 'SEASON', 'Quarter', 'Playoff', 'Semi', 'Final', 'Champion', 'SCORE', 'CONF', 'DIV',
                  'HOME', 'ROAD', 'OT', 'STREAK', 'LAST 10', 'TEAM_ID', 'EAST'], axis=1)
...
pred_concat = pd.concat([po_test[['EAST', 'TEAM', 'Playoff']].reset_index(), y_pred], axis=1)

po_pred = pred_concat.sort_values(['EAST', 'p1'], ascending=False).groupby('EAST').head(8)
...
winners = ['Philadelphia 76ers', 'Utah Jazz', 'Milwaukee Bucks', 'Atlanta Hawks', 'Denver Nuggets',
           'Los Angeles Clippers', 'Phoenix Suns', 'Brooklyn Nets']
...
pred_concat = pd.concat([team_names.reset_index(), y_pred], axis=1)
winners = ['Philadelphia 76ers', 'Milwaukee Bucks', 'Los Angeles Clippers', 'Phoenix Suns']
po_test[f"{stage}_pred"] = po_test['TEAM'].isin(winners)
